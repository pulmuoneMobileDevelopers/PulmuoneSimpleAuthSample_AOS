...
/*
 * 안드로이드 앱 별 KeyStore 의 Key를 통해 데이터를 암호화 - 복호화
 * base64 + 앱 별 고유키를 통한 암호화, 복호화
 */
object AndroidKeyStoreUtil {
    /*
     * 모든 입력값은 2048비트(256바이트) 보다 작음
     */
    ...
    fun init(applicationContext: Context) {
        ...
        val keyStore = KeyStore.getInstance(KEYSOTRE_INSTANCE_TYPE).apply { load(null) }
        ...
        val result: Boolean = if (keyStore.containsAlias(alias)) {
             ...
             */
            if (initAndroidM(alias)) { true }
            else { initAndroidL(alias) }
        }
